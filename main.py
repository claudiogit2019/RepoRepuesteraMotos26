...
if not st.session_state.autenticado:
    ...
    with st.container():
        sucursal_sel = st.selectbox("Sucursal / Rol:", ["Admin Global"] + list(st.session_state.db_total.keys()))
...
if st.session_state.rol == "admin":
    ...
    with tab1:
        ...
        for suc, items in st.session_state.db_total.items():
            for i in items:
                temp = i.copy(); temp['Sucursal'] = suc; lista_global.append(temp)
    ...
    with tab2:
        ...
        with col_adm1:
            ...
            with st.form("admin_add", clear_on_submit=True):
                ...
                n_sto = st.number_input("Stock", min_value=0.0)
        ...
        with col_adm2:
            st.write("### 🗑️ Editar o Eliminar")
            suc_edit = st.selectbox("Seleccionar Sucursal:", list(st.session_state.db_total.keys()), key="edit_suc")
            items_suc = st.session_state.db_total[suc_edit]
            prod_edit = st.selectbox("Seleccionar Producto:", [""] + [p['Producto'] for p in items_suc])
            if prod_edit:
                p_idx = next(i for i, p in enumerate(items_suc) if p['Producto'] == prod_edit)
                new_p = st.number_input("Nuevo Precio:", value=float(items_suc[p_idx]['Precio']))
                new_s = st.number_input("Nuevo Stock:", value=float(items_suc[p_idx]['Stock']))
                c1, c2 = st.columns(2)
...
# --- VISTA SUCURSAL ---
    ...
    with tabs[0]: 
        ...
        with col1:
            st.subheader("🎙️ COMANDO POR VOZ")
            ...
            if st.session_state.texto_ia:
                st.markdown(f'<div class="box-entendi">IA ENTENDIÓ:<br>{st.session_state.texto_ia}</div>', unsafe_allow_html=True)
                if st.button("✅ CARGAR A FACTURA", use_container_width=True):
            ...
            s_p = st.selectbox("BUSCAR REPUESTO:", [""] + sorted([p['Producto'] for p in inv_local]))
            if s_p:
                ...
                if st.button("➕ AÑADIR AL DETALLE"):
        ...
        with col2:
            ...
            if st.session_state.carrito:
                ...
                for idx, item in enumerate(st.session_state.carrito):
                    ...
                    cf1.write(f"**{item['Producto'].upper()}**")
                    cf2.write(f"x{round(item['Cant'], 2)}")
                    cf3.write(f"${item['Subtotal']:,.2f}")
                    if cf4.button("❌", key=f"del_{idx}"): st.session_state.carrito.pop(idx); st.rerun()
                ...
                if b1.button("⚡ FINALIZAR VENTA", use_container_width=True):
                    for it in st.session_state.carrito:
                        for p in inv_local:
                            if p['Producto'].lower() == it['Producto'].lower(): p['Stock'] = round(p['Stock'] - it['Cant'], 2)
                    guardar_datos(st.session_state.db_total); st.session_state.carrito = []; st.rerun()
                
                pdf_bytes = generar_ticket_pdf(st.session_state.carrito, total_f, paga, vuelto, st.session_state.sucursal_activa)
                b2.download_button("🖨️ TICKET PDF", data=pdf_bytes, file_name="ticket.pdf", mime="application/pdf", use_container_width=True)
                
                if b3.button("🔄 REINICIAR", use_container_width=True):
                    st.session_state.carrito = []; st.session_state.texto_ia = ""; st.rerun()
            else: st.info("La caja está vacía.")
